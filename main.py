...
def bind_sockets(interface):
    ...
    try:
        while True:
            ...
            dest_mac, src_mac, eth_protocol, eth_data = ethernet_frame(
                raw_data)  # gather the frame details
            src_port, dst_port = 0, 0
            if eth_protocol == 8:
                s_addr, d_addr, protocol, ip_header = ipv4_packet(
                    eth_data[14:34])
                ...
                if protocol == 6:
                    ...
                    src_port, dest_port, size, data = udp_packet(
                        eth_data[34:42])


                # BY DEFAULT ALL TRAFFIC IS ALLOWED. 
                # ANY ROUTES IN THE RULES FILE WILL BE DENIED
                if validate_with_route_table(s_addr, d_addr, src_port, dst_port):
                    ...
                    # print("[ERR] Route is either denied or invalid!")
                else:
...
if __name__ == "__main__":
    ...
    for key, val in interfaces.items():
        tr = threading.Thread(target=bind_sockets,
                              args=([key, val],), name=key)
